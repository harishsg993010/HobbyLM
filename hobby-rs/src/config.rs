...
pub struct Config {
    ...
    // ---- diffusion (LLaDA-style): bidirectional attention + iterative denoising decode ----
    pub diffusion: bool,           // true => generate by iterative denoising, not autoregression
    pub mask_token_id: usize,      // the [MASK] sentinel id used for the canvas
    pub block_size: usize,         // default semi-autoregressive block length for decode
}
...
impl Config {
    pub fn from_gguf(g: &Gguf) -> Result<Self> {
        ...
        let gating_sigmoid = match g.get_u32(&k("expert_gating_func")) {
            ...
            _ => true,        // hobbylm default in our export is sigmoid
        };
        ...
        Ok(Config {
            ...
            gating_sigmoid,
            diffusion: g.get_bool("diffusion.enabled").unwrap_or(false),
            mask_token_id: g.get_u32("diffusion.mask_token_id").map(|v| v as usize).unwrap_or(50257),
            block_size: g.get_u32("diffusion.block_size").map(|v| v as usize).unwrap_or(32),
            arch,
        })
    }
