...
(`bailingmoe2` arch). It loads the F32 GGUF directly and generates text — no llama.cpp, no Python at runtime.
...
```powershell
cargo build --release
.\target\release\moe-rs.exe --model ..\joint12-bailingmoe2.gguf --prompt "The capital of France is" --n 48 --temp 0
```

Flags: `--model <gguf>` `--prompt <s>` `--n <int>` `--temp <f>` (`0`=greedy) `--top-p <f>` `--seed <u64>` `--threads <n>`.

Get the F32 GGUF (~2 GB) from HF `rootxhacker/moe-omni-500m` (`export/joint12-bailingmoe2.gguf`) or the
Modal volume `fineweb10B:runs/500M_vlm_joint12/export/joint12-bailingmoe2.gguf`.
...
```powershell
moe-rs.exe --model joint12-bailingmoe2.gguf --image image_embeds.bin  --prompt "USER: Describe this image.`nASSISTANT:"
moe-rs.exe --model joint12-bailingmoe2.gguf --speech speech_embeds.bin --prompt "USER: What is being said?`nASSISTANT:"
```
